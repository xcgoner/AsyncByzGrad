...
for maxdelay in 5
do
    for nbyz in 0 2 4 6 8
    do
        logfile=$logdir/zenopp_${model}_${lr}_${method}_${maxdelay}_${byz}_${nbyz}.txt
        > $logfile
        for seed in 337 773 557 755 
        do
            python /homes/cx2/zeno_async/zeno_async/train_cifar10.py --classes 10 --model ${model} --nworkers 10 --nbyz ${nbyz} --byz-type ${byz} --byz-test ${method} --batchsize 128 --lr ${lr} --lr-decay 0.1 --lr-decay-epoch 100,150 --epochs 200 --seed ${seed} --max-delay ${maxdelay} --dir $inputdir --log $logfile 2>&1 | tee $watchfile
        done
    done
done
